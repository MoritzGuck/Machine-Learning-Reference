...
\begin{itemize}
    \item[] The \textbf{expected value} \index{expected value} ($E$) or mean ($\mu$) is given by $E[X] = \sum_{x \in X} x*p(x)$ for discrete RVs and $E[X] = \int_X x*p(x) dx$ for continuous RVs.
...
\paragraph{Binomial distribution} \index{Binomial distribution} jkjnk % todo complete
\paragraph{Bernoulli distribution} \index{Bernoulli distribution} jknk % todo complete
